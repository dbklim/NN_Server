...
class LeNet:
    ...
    def training(self):
        ...
        with self.default_graph.as_default():
            ...
            self.is_training = False
            self.init_classification_max()
            return "Обучение завершено. Точность классификации на тестовой выборке %.2f%%, время обучения: %.2f минут." % (scores[1]*100, (end_time - start_time)/60)
    ...
    def init_classification_max(self):
        ...
        # Компиляция модели
        self.model_classification_max.compile(loss="categorical_crossentropy", optimizer="adam", metrics=["accuracy"])
        ...
        if prediction == test_image_value:
            ...
            print("Error: тестовое изображение классифицировано неверно. Обучите сеть заново.")        
        pass
    ...
    def classification_max(self, recognize_image, client_addr):
        ...
        if self.is_training == True:
            print("%s:%s | Сеть недоступна, выполняется обучение сети." % (client_addr[0], client_addr[1]))
            return "-1"
        ...
        prediction = self.model_classification_max.predict(x)
        prediction = int(numpy.argmax(prediction))
        print("%s:%s | На изображении цифра %d, время распознавания: %f секунд." % (client_addr[0], client_addr[1], prediction, (time.time() - start_time)))
        return prediction