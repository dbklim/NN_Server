...
class ClientWorking(Thread):
    ...
    def run(self):
        ...
        try:
            ...
            print('\n%s [INFO] | Подключился клиент %s:%s' % (datetime.now(), self.client_addr[0], self.client_addr[1]))
            while True:
            ...
            print("%s [ERROR] | %s:%s | Error: %s" % (datetime.now(), self.client_addr[0], self.client_addr[1], e))
    ...
    def __POST(self, header, data):
        ...
        type_neural_network = header[header.find("POST") + 5:header.find("HTTP") - 1]
        type_neural_network = type_neural_network[type_neural_network.find("/") + 1:]
        
        type_operation = type_neural_network[type_neural_network.find("/") + 1:]
        type_neural_network = type_neural_network[:type_neural_network.find("/")]
        ...
        if type_neural_network == "lenet":
            if type_operation == "classification_max":
                ...
                print("%s:%s | Инициировано обучение сети с параметрами по умолчанию на выборке mnist." % (self.client_addr[0], self.client_addr[1]))
                return self.__create_response(normal_response_header, "\r\n\r\n", self.lenet.training())
            else:
    ...
    def __create_response(self, part1, part2, data = None):
        ...
        if data != None:
            return part1 + str(len_part1 + len_part2 + len(len_response)) + part2 + data
        else:
            return part1 + str(len_part1 + len_part2 + len(len_response)) + part2